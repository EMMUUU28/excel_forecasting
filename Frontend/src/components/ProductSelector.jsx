...
import { enhanceProductData } from "../data/product-details-generator";
...
// Enhance the product data with additional details
const enhancedProductData = enhanceProductData(originalProductData);
...
function ProductSelector() {
  ...
  useEffect(() => {
    ...
    const categoryData = originalProductData[selectedCategory];
  ...
  const handleProductSelect = (productId) => {
    const productDetails = enhancedProductData[selectedCategory].productDetails[productId];
    setSelectedProduct({
  ...
  if (selectedProduct) {
    return (
      <ProductDetailsView 
        product={selectedProduct.details}
        ...
        <div className="md:col-span-3 bg-gray-50 p-4 border-r border-gray-200">
          <div className="mb-6">
            ...
            <div className="relative">
              ...
              {isCategoryDropdownOpen && (
                <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-80 overflow-y-auto">
                  {Object.keys(originalProductData).map((category) => (
                    <button
                      key={category}
                      className={`w-full text-left p-3 hover:bg-indigo-50 transition-colors ${
                        selectedCategory === category ? "bg-indigo-50 text-indigo-700" : ""
              ...
              <div className="space-y-2">
                {allFilters.map((filter) => (
                  ...
                  >
                    ...
                    <span className="text-sm">{filter.replace("Pid_to_", "")}</span>
                    <span className="ml-auto text-xs text-gray-500">
                      {originalProductData[selectedCategory] && 
                       originalProductData[selectedCategory][filter] ? 
                       originalProductData[selectedCategory][filter].length : 0}
                    </span>
                  </div>
                ))}
              </div>
